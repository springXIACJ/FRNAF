...
from render import render_rays, run_network
...
def config_parser():
    ...
    parser.add_argument("--config", default="./config/left_knee_50.yaml",
...
class BasicTrainer(Trainer):
    ...
    def compute_loss(self, data, global_step, idx_epoch):
        ...
        ret = render_rays(rays,idx_epoch,self.net,self.net_fine, **self.conf["render"])
        projs_pred = ret["acc"]
    ...
    def eval_step(self, global_step, idx_epoch):
        ...
        for i in range(0, rays.shape[0], self.n_rays):
            projs_pred.append(render_rays(rays[i:i+self.n_rays], idx_epoch,self.net, self.net_fine, **self.conf["render"])["acc"])
        projs_pred = torch.cat(projs_pred, 0).reshape(H, W)

        # Evaluate density
        image = self.eval_dset.image
        image_pred = run_network(self.eval_dset.voxels,idx_epoch, self.net_fine if self.net_fine is not None else self.net, self.netchunk)

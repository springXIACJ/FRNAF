...
def get_ssim_3d(arr1, arr2, size_average=True, PIXEL_MAX=1.0):
    ...
    ssim_d = []
    for i in range(N):
        ssim = structural_similarity(arr1_d[i], arr2_d[i],data_range=1)
        ssim_d.append(ssim)
    ...
    ssim_h = []
    for i in range(N):
        ssim = structural_similarity(arr1_h[i], arr2_h[i],data_range=1)
        ssim_h.append(ssim)
    ...
    ssim_w = []
    for i in range(N):
        ssim = structural_similarity(arr1[i], arr2[i],data_range=1)
        ssim_w.append(ssim)

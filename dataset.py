...
class TIGREDataset(Dataset):
    ...
    TIGRE dataset.
    """
    def __init__(self, path,n_rays, n_patch,patch_size, type="train", device="cuda"):    
        super().__init__()
        ...
        if type == "train":
            ...
            self.coords = torch.stack(torch.meshgrid(torch.linspace(0, self.geo.nDetector[1] - 1, self.geo.nDetector[1], device=device),
                                                torch.linspace(0, self.geo.nDetector[0] - 1, self.geo.nDetector[0], device=device), indexing="ij"),
                                 -1)
            self.ray_coods=torch.reshape(self.coords, [-1, 2])
    ...
    def __getitem__(self, index):
        if self.type == "train":
            nonzero_indices = torch.nonzero(self.projs[index]>0)
            projs_valid = (self.projs[index]>0).flatten()
            coords_valid = self.ray_coods[projs_valid]
            ...
            selected_indices = torch.randperm(nonzero_indices.size(0))[:self.n_patch]
            patch_indices_all = []
            for i in selected_indices:
                start_h, start_w = nonzero_indices[i]
                # 确保 patch 不会超出原始张量的边界
                start_h = max(0, min(start_h, self.geo.nDetector[1] - self.patch_size))
                start_w = max(0, min(start_w, self.geo.nDetector[0] - self.patch_size))
                end_h = start_h + self.patch_size
                end_w = start_w + self.patch_size
                patch_indices=self.coords[start_h:end_h, start_w:end_w]
                patch_indices=torch.reshape(patch_indices, [-1, 2])
                patch_indices_all.append(patch_indices)
            patch_indices_all = torch.stack(patch_indices_all)
            patch_indices_all=torch.reshape(patch_indices_all, [-1, 2]).long()
            patch_indices_all=torch.concat([patch_indices_all,select_coords],dim=0)
            rays = self.rays[index, patch_indices_all[:, 0], patch_indices_all[:, 1]]
            projs = self.projs[index, patch_indices_all[:, 0], patch_indices_all[:, 1]]
            # projs = torch.reshape(projs, [self.n_patch, self.patch_size, self.patch_size])
            out = {
                "projs":projs,
                "rays":rays,
            }

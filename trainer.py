...
from loss import NormalizedCrossCorrelation2d as ncc

class Trainer:
    def __init__(self, cfg, device="cuda:2"):
        ...
        self.n_patch=cfg["train"]["n_patch"]
        ...
        # Dataset
        train_dset = Dataset(cfg["exp"]["datadir"],self.n_rays,self.n_patch,self.patch_size, "train", device)
        self.eval_dset = Dataset(cfg["exp"]["datadir"],self.n_rays,self.n_patch,self.patch_size, "val", device) if self.i_eval > 0 else None
        ...
        self.nccloss=ncc()
        print("train_loader:",len(self.train_dloader))
